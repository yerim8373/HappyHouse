                        ...
                        $.ajax({
                            ...
                            <hr>`;
                                    ...
                                    infowindow = new kakao.maps.InfoWindow({
                                        content: `<div><span>\${house.aptName}</span>
                            <br>
                            <span>거래금액 : \${house.dealAmount}</span><br>
                            <span> 전용면적 : \${house.area} </span><br>
                            <span> 층 : \${house.floor} </span></div>` // 인포윈도우 내부에 들어갈 컨텐츠 입니다.
                                    ...
                                    kakao.maps.event.addListener(marker, 'click', function () {
                                        let store = $("#store").val();
                                        if (store === '') {
                                            alert("검색할 상점 이름을 입력해 주세요!");
                                            return;
                                        }
                                        $.ajax({
                                            //url: "/store/nearby/?aptCode=" + house.aptCode,
                                            //data: {'act': 'search2', "aptCode":`\${house.aptCode}`},
                                            url: "/store/nearby/" + store + "?lat=" + house.lat + "&lng=" + house.lng + "&dist=0.5",
                                            type: 'GET',
                                            success: function (response) {
                                                let storeBounds = new kakao.maps.LatLngBounds();
                                                code = ``;
                                                console.log(response);
                                                setStoreMarker(null);
                                                $.each(response, function (i, s) {
                                                    ...
                                                    code += `
                                                            <h5>\${s.name}</h5><br>
                                                            <span> 업종 : \${s.type} </span><br>
                                                            <hr>`;
                                                    let position = new kakao.maps.LatLng(Number(`\${s.lat}`), Number(`\${s.lng}`));
                                                    let storeMarker = new kakao.maps.Marker({ map: map, position: position, clickable: false });
                                                    storeBounds.extend(position);
                                                    storeMarkers.push(storeMarker);
                                                });
                                                setStoreMarker(map);
                                                map.setBounds(storeBounds);
                ...
                <div class="container body">
                    ...
                    <div class="row col-lg-12 w-100">
                        ...
                        <div class="col-lg-3">
                            <h4>주변 상점 정보</h4>
                            <hr>
                            <div class="infobox" id="infobox" style="height: 680px; overflow: auto;"></div>
                        </div>
                                ...
                                <div class="mb-2 justify-content-center">
                                    <div class="d-flex w-100">
                                        ...
                                        <form id="sform" class="w-50 form-inline justify-content-end">
                                            <input class="form-control mr-2 w-70" id="store" name="store" type="text" />
                                        </form>
                                    </div>
                                </div>
                            </div>
                            <div id="map" style="width: 100%; height: 720px; margin: auto" class="mb-3"></div>
                            <!-- Kakao Map end -->
                        </div>
                    </div>
